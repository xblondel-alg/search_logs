...
This repository search timestamped queries on a period of time.
...
The dataset is loaded when the application starts.
...
In this version, we did not pay much attention to the API implementation. Most notably:

- Route and route versions are hardcoded (we could have used blueprints).
- Any execution error will result in a 500 raw error. A cleaner implementation would format an error response.
...
The same module also exposes a `parse_timestamp` function that parses dates contained in the dataset.
...
I initially started with a binary search tree, aiming to convert it to an n-ary search tree (more than 2 child nodes for each node), but I reconsidered and decided to tackle the underlying data structure. Namely:
...
- We have year nodes, that contains months. Each year node contains up to 12 month nodes.
- We have month nodes, that contain days. Each month node contains up to 31 day nodes.
- We have day nodes, that contain hours. Each day node contains up to 24 hour nodes.
- We have hour nodes, that contain minutes. Each hour node contains up to 60 minute nodes.
...
The `Node` class contains a start and end `datetime` values, which constitute an interval where end is excluded. It also provides us with an `_overlaps_interval` method, that checks whether a given interval matches the start and end of the node; it is used for searching in the tree.

The `SearchTree` node actually contains a dictionary, associating a year to a `YearNode` instance.

Adding a value to a `SearchTree` node is simply implemented by finding or creating the matching year node, then call the adding method of this node.
...
The `MinuteNode` is at the leaf of the tree, and contains the data in an unordered list, as we systematically return the whole data set if the interval is matched when searching.

Adding a value to the minute node simply adds it to the list. Searching checks for interval overlap, and returns the whole list, as we already are at the last level of the interval. Note that we ignore the seconds of the timestamp in the dataset file.
...
The implementation for getting the count is in the `get_distinct_count_from_dataset` method, that simply counts the keys of the dataset.
...
All these algorithms work with in-memory data structures. Solutions exist to be able to swap then in and out at will, but they would require a few days work to be implemented.

## Memory consumption of the search tree

The search tree stores all data in memory. This could be improved in two ways, than would require a few days to explore.
...
We would need to devise some heuristics to correctly implement this approach. Most notably:
...
- Can we have a persistence scheme that can also be applied to temporary data structures, such as the ones used in the count algorithms above?

Examining these heuristics would require a few days and some experiments.
