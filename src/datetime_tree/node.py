...
class Node(ABC, Generic[TDataType]):
    ...
    """
    def __init__(self, level_value: int, max_children_number: int, children_index_base: int) -> None:
        """
            Constructor.
...
class NonDataNode(Node[TDataType]):
    ...
    def __init__(self, level_value: int, max_children_number: int, children_index_base: int) -> None:
        ...
        self._children: List[Optional[Node[TDataType]]] = [None for _ in range(self._max_children_number)]
...
class MinuteNode(Node[TDataType]):
    ...
    def add_value(self, key: datetime, value: TDataType) -> None:
        ...
        if key.minute != self._level_value:
            raise ValueError(f"Trying to insert key {key} at minute {self._level_value}")
